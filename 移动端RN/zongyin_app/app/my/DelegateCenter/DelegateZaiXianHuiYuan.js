...
const titList = ['积分','金额','QQ','最近登陆','状态'] 
...
export default class DelegateZaiXianHuiYuan extends React.Component {
    ...
    render(){
        ...
        if (this.state.dataList.length) {
            ...
            text = {this.state.isloading?"加载中":"暂无会员在线,点击重新加载"}
        />
        }
            ...
            <ScrollView horizontal={true}>
                ...
                {titList.map((titStr, index)=>{
                     ...
                     {this.state.xianShiArr.map((info,weizNum)=>{
                        ...
                        var contentStr ='' 
                        if (index==0) {
                            contentStr = info.point
                        } else if (index==1) {
                            contentStr = info.balance
                        } else if (index==2) {
                            contentStr = info.qq
                        } else if (index==3) {
                            contentStr = info.logintime
                        } else if (index==4) {
                            contentStr = info.isonline ? '在线' : '离线'
                        } 
