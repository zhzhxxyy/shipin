...
const titList = ['账户余额','充值金额','提款金额','投注额','中奖额',
                 '佣金','活动','手续费','棋牌','盈亏']
...
export default class DelegateBaoBiao extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            yeNum:0,
            xianShiArr:[],
            xianShiNum:parseInt((sys.dheight*4/6 - 45 - 35*2)/35)
        }
    }
  ...
  _createDateData(){
   ...
   _showDatePicker(num) {
    ...
    renderHeadView(){
        ...
        let totalView = <View style={{height:10}}></View>;
        // if (isCunZai) {
        ...
        <View style={{marginTop:10,marginLeft:0,width:sys.dwidth,height:42,
            ...
            <TouchableOpacity disabled={this.state.isChaXun} style={{width:(sys.dwidth-40)/3,
                marginLeft:10, marginTop:0,height:40,backgroundColor:sys.mainColor,borderRadius:5,
                flexDirection:'row',}} onPress={()=>this.chazhao()}>
                {this.state.isChaXun?<ActivityIndicator style={{marginLeft:((sys.dwidth-40)/3-60)/2 - 10 - 20,width:20
                ...
                },{width:60,marginLeft:this.state.isChaXun?10:((sys.dwidth-40)/3-60)/2}]}>{this.state.isChaXun?'查询中..':'查询'}</Text>
            </TouchableOpacity>
    ...
    itemView(inde){
        ...
        <View style={{flexDirection: 'row'}}>
            ...
            <View>
                   ...
                   <View>
                    ...
                    <View style={{borderRightWidth:1,borderColor:sys.grayColor,borderBottomWidth:1}}>
                     <Text style={{marginLeft:10,marginRight:10,textAlign:'center'
                        ...
                        }}>{'总计'+'('+this.state.dataList.length+'人)'}</Text>
                        </View>
            ...
            <ScrollView horizontal={true}>
                ...
                {titList.map((titStr, index)=>{
                    ...
                    if (this.state.totaldata != null) {
                        if (index==0) {
                            totalStr = this.state.totaldata['balance']
                        } else if (index==1) {
                            totalStr = this.state.totaldata.chongzhi
                        } else if (index==2) {
                            totalStr = this.state.totaldata.tixian
                        } else if (index==3) {
                            totalStr = this.state.totaldata.touzhu
                        } else if (index==4) {
                            totalStr = this.state.totaldata.zhongjiang
                        } else if (index==5) {
                            totalStr = this.state.totaldata.yongjin
                        } else if (index==6) {
                            totalStr = this.state.totaldata.huodong
                        } else if (index==7) {
                            totalStr = this.state.totaldata.shouxufei
                        } else if (index==8) {
                            totalStr = this.state.totaldata.kyyingkui
                        } else if (index==9) {
                            totalStr = this.state.totaldata.yinli
                        } 
                    }
                    ...
                    <View style={{borderLeftWidth:index==0?0:1,borderColor:sys.grayColor,borderBottomWidth:1}}>
                     <Text style={{marginLeft:10,marginRight:10,textAlign:'center'
                         ,height:35,color:sys.titleColor,textAlignVertical:'center'
                         ,backgroundColor:sys.whiteColor,...Platform.select({
                             ios: { lineHeight: 35},
                     ...
                     {this.state.xianShiArr.map((info,weizNum)=>{
                        ...
                        var contentStr ='' 
                        if (index==0) {
                            contentStr = info.balance
                        } else if (index==1) {
                            contentStr = info.chongzhi
                        } else if (index==2) {
                            contentStr = info.tixian
                        } else if (index==3) {
                            contentStr = info.touzhu
                        } else if (index==4) {
                            contentStr = info.zhongjiang
                        } else if (index==5) {
                            contentStr = info.yongjin
                        } else if (index==6) {
                            contentStr = info.huodong
                        } else if (index==7) {
                            contentStr = info.shouxufei
                        } else if (index==8) {
                            contentStr = info.kyyingkui
                        } else if (index==9) {
                            contentStr = info.yinli
                        } 
       ...
       return<View>
               ...
               <View style={[{backgroundColor:'white',marginTop:5,width:sys.dwidth - 100 ,},{width:sys.dwidth,marginTop:0}]}>
                     <Text numberOfLines={1} style={{marginLeft:10,fontSize:14,marginTop: 8,color:'#333333',lineHeight:20,}}>{"最后登陆时间:"+follow.logintime}</Text>


                     <View style={{flexDirection:'row',width:sys.dwidth}}>
                         <Text style={{marginLeft:10,fontSize:14,marginTop: 8,color:'#333333',lineHeight:20,width:sys.dwidth/3-10}}>{"充值:"+follow.chongzhi}</Text>
                         <Text style={{marginLeft:10,fontSize:14,lineHeight:20,marginTop: 8,color:'#333333',width:sys.dwidth/3-10,}}>{"提款:"+follow.tixian}</Text>
                    </View>

                     <View style={{flexDirection:'row',width:sys.dwidth}}>
                         <Text style={{marginLeft:10,fontSize:14,marginTop: 8,color:'#333333',lineHeight:20,width:sys.dwidth/3-10}}>{"投注:"+follow.touzhu}</Text>
                         <Text style={{marginLeft:10,fontSize:14,marginTop: 8,color:'#333333',lineHeight:20,width:sys.dwidth/3-10}}>{"返奖:"+follow.zhongjiang}</Text>
                         {/* <Text style={{marginLeft:10,fontSize:14,marginTop: 8,color:'#333333',lineHeight:20,width:sys.dwidth/3-10}}>{"盈利:"+follow.yinli}</Text> */}
                     </View>

                     <View style={{flexDirection:'row',width:sys.dwidth}}>
                         <Text style={{marginLeft:10,fontSize:14,marginTop: 8,color:'#333333',lineHeight:20,width:sys.dwidth/3-10}}>{"佣金:"+follow.yongjin}</Text>
                         <Text style={{marginLeft:10,fontSize:14,lineHeight:20,marginTop: 8,color:'#333333',width:sys.dwidth/3-10,}}>{"活动:"+follow.huodong}</Text>
                     </View>
                     <View style={{flexDirection:'row',width:sys.dwidth}}>
                         <Text style={{marginLeft:10,fontSize:14,marginTop: 8,color:'#333333',lineHeight:20,width:sys.dwidth/3-10}}>{"手续费:"+follow.shouxufei}</Text>
                         <Text style={{marginLeft:10,fontSize:14,lineHeight:20,marginTop: 8,color:'#333333',width:sys.dwidth/3-10,}}>{"开元棋牌:"+follow.kyyingkui}</Text>
                     </View>
                     <View style={{flexDirection:'row',width:sys.dwidth}}>
                         <Text style={{marginLeft:10,fontSize:14,lineHeight:20,marginTop: 8,color:'#333333',width:sys.dwidth/3-10,}}>{"盈利:"+follow.yinli}</Text>
                     </View>
                </View>
        ...
        HttpUtils.myPost(codeurl,formData)
            .then(result=>{
                ...
                if(result['respCode'] ==1) {
                    ...
                    var newArr=[]
                    if (testData.length>this.state.xianShiNum) {
                        
                        for(var i=0;i<this.state.xianShiNum;i++) {
                            newArr.push(testData[i])
                        }
                    ...
                    this.setState({
                        ...
                        xianShiArr:newArr,
                        totaldata:totaldata,
                        refreshState:RefreshState.NoMoreData,// (testData.length >= result['data']['total']) ? RefreshState.NoMoreData : RefreshState.Idle,
                    })
                } else {
