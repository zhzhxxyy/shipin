...
const titList = ['彩种及期号','投注金额','中奖金额','玩法','下注情况','开奖结果','状态']
...
//彩票状态
  ...
  componentWillUnmount(){
    ...
    itemView(){
       ...
       // var myHeigth = sys.dheight*4/7 - 45
                ...
                {titList.map((titStr, index)=>{
                     ...
                     {this.state.xianShiArr.map((info,weizNum)=>{
                        ...
                        if(info.isdraw==1){
                            typetit = '已中奖';
                            // color = sys.redColor
                        }else if(info.isdraw==-1){
                            typetit = '未中奖';
                            // color = sys.blueColor
                         }

                        // '彩种及期号','投注金额','中奖金额','玩法','下注情况','开奖结果'
                        var myStyle={}
                        var contentStr ='' 
                        if (index==0) {
                            contentStr = info.cptitle+"("+info.expect+")"
                        } else if (index==1) {
                            contentStr = info.amount
                        } else if (index==2) {
                            contentStr = info.okamount
                        } else if (index==3) {
                            contentStr = info.playtitle+"（"+info.mode+"）"
                        } else if (index==4) {
                            myStyle = {width:sys.dwidth/4}
                            contentStr = info.tzcode
                        } else if (index==5) {
                            contentStr = info.opencode
                        } else if (index==6) {
                            contentStr = typetit
                        } 

                         return <TouchableOpacity onPress={()=>{this.itemClick(info)}}>
