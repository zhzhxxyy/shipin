...
//浏览器打开url
public class NewWebViewActivity extends ToolBarBaseActivity implements View.OnClickListener {
                ...
                if (url.startsWith("mqqwpa")) {
                    //qq 客服
                      ...
                      for(int i=0;i<all.length;i++){
                          if(!StringUtils.isEmpty(all[i])){
                              String[] item=all[i].split("=");
                              if(item.length==2&&"uin".equals(item[0])){
                                  qq=item[1];
                                  break;
                              }
                          }
