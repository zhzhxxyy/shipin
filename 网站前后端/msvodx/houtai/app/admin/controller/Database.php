...
use app\common\util\Databases as dbOper;
use think\Db;
/**
 * 数据库管理控制器
...
class Database extends Admin
{
    ...
    public function index($group = 'export')
    {
        ...
        if ($group == 'export') {
            $data_list = $this->myDb->query("SHOW TABLE STATUS");
        } else {
            //列出备份文件列表
    ...
    public function optimize($ids = '')
    {
        ...
        $res = $this->myDb->query("OPTIMIZE TABLE `{$tables}`");
        if ($res) {
    ...
    public function repair($ids = '')
    {
        ...
        $tables = implode('`,`', $table);
        $res = $this->myDb->query("REPAIR TABLE `{$tables}`");
        if ($res) {
