 ...
 * @return bool true:数据库配置信息正确,other:错误
 * @throws Exception 数据库连接错误信息
 * @Author: xiangzhanyou
 * @Date:   2017-11-14
 */
function x_db_config($config)
{
    if (!is_array($config)) return false;

    $clientDb = \think\Db::connect($config);

    try {
        $clientDb->execute('select version()');
    } catch (Exception $err) {
 ...
 * 连接webDatabase
 */
function x_connect_webdatabase()
{
    $myDb = null;
 ...
 * @param $name 配置项名称
 */
function x_get_webseting($name, $isGroup = false)
{
    $config = null;

    if ($isGroup) {
        $config = x_connect_webdatabase()->name('admin_config')->where("`group`='$name'")->select();
        $returnData = [];
        if ($config) {
        ...
        $config = x_connect_webdatabase()->name('admin_config')->where("name='$name'")->find();
        return $config['value'];
    }
...
function check_admin_auth(){
    ...
    $authStr = request()->module() . "/" . request()->controller() . "/" . request()->action();

    $allowController=['index','publics','user'];

    if(in_array(strtolower(request()->controller()),$allowController)) return true;


    $user = session('admin_user');
    #if($user['uid']==1) return true;//超级管理员
    if ($user) {
        $userRoleId = $user['version'];
        $userRoleInfo =\think\Db::name('version_role')->where("id=$userRoleId")->find();
        $userRolesArr = \json_decode($userRoleInfo['auth']);
        //查询当前authStr数据库对应的id
        $curAuthInfo = \think\Db::name("admin_menu")->where(['url' => $authStr])->find();
        $curAuthId = $curAuthInfo['id'];
        if (!in_array($curAuthId, $userRolesArr)) {
            return false;
        }else{
            return true;
        }
...
/** 云转码播放密钥生成 **/
function create_yzm_play_sign($key)
{
    $key = trim($key);
    if (empty($key)) return '';
    $time = time() . '000';
...
/**
 * 会员密码加密算法,如果要改动的话，客户端前端算法方法也要修改
 * @author Dreamer
 * @param $pwd
 * @return string
 */
