...
jQuery(function() {
                ...
                $.ajax({
                    ...
                    if (data.ifExist) { //若存在，这返回失败给WebUploader，表明该文件不需要上传
						...
						if(data.share){
							...
							$("#img_video_thumb").attr('src',data.pic);
    ...
    uploader = WebUploader.create({
        ...
        accept: {
            ...
            mimeTypes: '.rmvb,.flv,.vob,.mp4,.mov,.3gp,.wmv,.mp3,.mkv,.mpg,.ts,.avi,.mpeg,.avi,.rm,.wav,.asf,.divx,.mpg,.mpe,.vod,.mts'
        },
        duplicate: false,
        chunked: true,
        chunkSize: chunkSize,
