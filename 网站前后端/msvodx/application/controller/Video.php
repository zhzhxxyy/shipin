...
//            $where .= " and tag = $tag_id";
    ...
    public function syncAddVideo()
    {
        ...
        file_put_contents(dirname(__FILE__)."../../../syncAddVideo.log","\n".str_repeat('-*-',50).$task,FILE_APPEND);
        if ($task) {
            //logError('视频数据:'.$task);
            ...
            if ($json) {
                if ($json == "key error.") {
                    file_put_contents(dirname(__FILE__)."../../../syncAddVideo.log","API密钥错误");
                    die;
                }
    ...
    //异步通知视频入库
    public function yzmVideo()
    {
     
        
      $data  =urldecode(file_get_contents("php://input"));
      file_put_contents('./dongsheng.txt',$data);
      $data = self::param($data);
      
      
      $url='http://www.zhzhxxyy.com/video/m3u8/';
      $video_list = Db::name('video')->where(array('url' => $url.$data['m3u8']))->field('id')->select();
      if(!empty($video_list)){
            echo '该视频已经同步过' ;
            die;
      }
