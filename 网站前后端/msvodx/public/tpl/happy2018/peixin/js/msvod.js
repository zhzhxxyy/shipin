...
//低版本浏览器提示
(function () {
$('.upbrower').on('click', '.close', function() {
...
$('.header-search .txt').css('width', '220px');
});
...
$('.header-search .txt').css('width', '200px');
}
});
...
window.location.href = '/search/index/type/video.html?key_word=' + $key_word + '&identify=' + identify;
});
...
toLogin();
$("#login-box .tab-head .t1").addClass("active").siblings().removeClass("active");
$("#loginModal-t2").hide();
$("#loginModal-t1").show();
});
...
$('#registerBtn').on('click', function () {
toLogin();
$("#login-box .tab-head .t2").addClass("active").siblings().removeClass("active");
$("#loginModal-t1").hide();
$("#loginModal-t2").show();
});

//登录注册切换
$("#login-box .tab-head").on("click", ".t1", function () {
$(this).addClass("active").siblings().removeClass("active");
$("#loginModal-t1").show();
$("#loginModal-t2").hide();
});
$("#login-box .tab-head").on("click", ".t2", function () {
$(this).addClass("active").siblings().removeClass("active");
$("#loginModal-t2").show();
$("#loginModal-t1").hide();
});
...
$('.phone-m').find('.captchaimg').attr('src', data.url);
});
})
// 验证码发送60秒倒计时
...
// 点击发送验证码
$("#login-box .get-p").click(function () {
var that = $(this);
// 点击发送验证码后验证手机号码格式
...
$.get(Url.sendMessage, {'phone': phone, 'verifyCode': verifyCode}, function (res) {
...
// 点击切换验证码
$("#login-box .captchaimg").click(function () {
var unique = new Date().getTime();
var that = $(this);
$.get('/site/captcha', {'refresh': 1, 'v': unique}, function (data) {
...
// 点击登录（或注册）
$('#login-box .phone-btn').click(function () {
var that = $(this);
var data = {};
data.phone = $(this).parents('.phone-m').find('.phone-num input').val();
data.code = $(this).parents('.phone-m').find('.phone-yz .yz-l input').val();
if (!checkTelNum(data.phone)) {
that.parents('.phone-m').find('.phone-num').addClass('error');
return false;
}
...
//设计师统计
var isDesiger = $('#isDesiger'), yLevel = $('#yLevel');

$('.survey-wid .closeBtn').on('click', function () {
$.post(Url.cookiedesign, {type: 1}, function () {
});
$('.survey-wid').fadeOut();
});
testClick();
//判断是不是已选择状态
function testClick() {
var k = $('.survey-wid .main-cont .elect-item a').hasClass('on');
$('.survey-wid .main-cont .elect-item a').on('click', function () {
if (!k) {
$(this).addClass('on').siblings('a').removeClass('on');
ckeck();
}
...
//当全部选择时，可提交按钮变为可提交状态
function ckeck() {
if (isDesiger.find('a').hasClass('on') && yLevel.find('a').hasClass('on')) {
$('.survey-wid .submit-box .submitBtn').addClass('canSubmit');
$('.survey-wid .submit-box .tips').fadeOut();
} else {
return false;
}
}
...
//提交时的状态提示与提交成功时的切换
$('.survey-wid .submit-box .submitBtn').on('click', function () {
var qq = $("input[name=qq]").val();
if (!isDesiger.find('a').hasClass('on')) {
$('.survey-wid .submit-box .tips').fadeIn().text('请填写“您是设计师吗？”');
return false;
} else if (!yLevel.find('a').hasClass('on')) {
$('.survey-wid .submit-box .tips').fadeIn().text('请填写“您觉得自己当前位于？”');
} else if (!qq) {
$('.survey-wid .submit-box .tips').fadeIn().text('请填写“您的qq”？”');
} else {
var grade = $(this).parent().siblings("#yLevel").children(".on").attr("data");
var is_design = $(this).parent().siblings("#isDesiger").children(".on").attr("data");
...
$.post(Url.design, data, function (data) {
if (data.isError) {
$('.survey-wid .elect-cont').fadeOut();
$('.survey-wid .error-cont').fadeIn();
} else {
$('.survey-wid .elect-cont').fadeOut();
$('.survey-wid .success-cont').fadeIn();
setTimeout(function () {
$('.survey-wid').fadeOut();
}, 1000)
}
...
var dItem_1 = $('.oitem-list[data-item="0"]');
var dItem_2 = $('.oitem-list[data-item="1"]');
$('.option-item .oitem-list .li-box').on('click', function () {
if (!$(this).hasClass('on')) {
$(this).addClass("on").siblings(".li-box").removeClass('on')
}
if ($(this).attr("data-is") == 0) {
$(dItem_1).hide();
$(dItem_2).show();

} else if ($(this).attr("data-is") == 1) {
$(dItem_1).show();
$(dItem_2).hide();
}
if ($('.survey-body .option-item:eq(0) .li-box.on').length != 0) {
$('.survey-body .option-item:eq(0) .w-tips').addClass('hide');
}
if ($('.survey-body .option-item:eq(1) .li-box.on').length != 0) {
$('.survey-body .option-item:eq(1) .w-tips').addClass('hide');
}
if ($('.survey-body .option-item:eq(2) .li-box.on').length != 0) {
$('.survey-body .option-item:eq(2) .w-tips').addClass('hide');
}
if ($('.survey-body .option-item:eq(0) .li-box.on').length != 0 &&
$('.survey-body .option-item:eq(1) .li-box.on').length != 0 &&
$('.survey-body .option-item:eq(2) .li-box.on').length != 0) {
$('.survey-botm .survey-submit-btn').removeClass('disabled');
}
});
$('.survey-botm .survey-submit-btn').on('click', function () {

var is_design = $('.survey-body .option-item:eq(0) .li-box.on').attr("data-is");

var grade = $('.survey-body .option-item:eq(1) .li-box.on').attr("data-level");

var job = $('.survey-body .option-item:eq(2) .li-box.on').attr("data-job");

var qq = $('.survey-body .option-item .contact-way-put').val();

if ($('.survey-body .option-item .li-box.on').length == 0) {
$('.survey-body .option-item .w-tips').removeClass('hide');
return false;
}
if (is_design == undefined) {
$('.survey-body .option-item:eq(0) .w-tips').removeClass('hide');
return false;
}
if (grade == undefined) {
$('.survey-body .option-item:eq(1) .w-tips').removeClass('hide');
return false;
}
if (job == undefined) {
$('.survey-body .option-item:eq(2) .w-tips').removeClass('hide');
return false;
}

if (qq != '' && isNaN(qq)) {
$('.survey-body .option-item:eq(3) .w-tips').removeClass('hide');
return false;
}
...
$('.album-collect-success').addClass('hide').css({
top: b,
left: s
});
}
...
data: {id: dataId, passive_id:$album._config.passive_id,type:$album._config.type, '_csrf-frontend': $('meta[name="csrf-token"]').attr('content')},
dataType: 'JSON',
type: 'post',
success: function (response) {
if(response.error === 0){
...
$.ajax({
url : url,
data: {id: id, passive_id:$album._config.passive_id,type:$album._config.type, '_csrf-frontend': $('meta[name="csrf-token"]').attr('content')},
dataType: 'JSON',
type: 'post',
success: function (response) {
if(response.error === 0){
...
/* COMBO: filename = js/album/album.js, type = application/javascript, resp = 200 */
$(function(){
var $ = jQuery;

// 分类展开收起
var navh    = $('.album-nav .wrap-m').height();
moreBtn = $('.album-nav .more-btn');
if (navh > 60) {
moreBtn.show();
}
$('.album-nav').on('click','.more-btn',function(){
var itemh = $('.album-nav .nav-m').height();
if (itemh < 60) {
$(this).addClass('active').parents('.nav-m').css('height', 'auto');

}else{
$(this).removeClass('active').parents('.nav-m').css('height', '40px');
}
});
// 编辑有默认内容的输入字符倒数
function st(){
// var lh = $('.input-txt input').val().length;
//     bh = $('.textarea textarea').text().length;
// $('.input-txt .surplus-num i').text(lh);
// $('.textarea .surplus-num i').text(bh);
}
st();
// 添加标签多选判断
$('.input-r').on('click','.tags span',function(){
var acNum = $('.input-r .tags').find('span.active').length;
isC   = $(this).hasClass('active');
if (isC == true) {
...
// 输入字符倒数
function wordsDeal(dat) {
var curLength = $(dat).val().length;
maxNum    = $(dat).attr('data-num');
if (curLength > maxNum) {
// var num = $(dat).val().substr(0, maxNum);
//$(dat).val(num);
} else {
$(dat).siblings('.surplus-num').find('i').text(curLength);
}
}


$("#album .hover-album").on('mouseover', function () {
...
$("#album .hover-album").on('mouseout', function () {
var $edit = $(this).find('.edit');
if(!$edit.hasClass('hide')){
$edit.addClass('hide');
...
var relation_id = $(this).attr('data-ralation');
$(".base-win .del").attr('data-relation-id', relation_id);
$(".base-win").removeClass('hide');

});

$('.base-win').on('click', '.close', function() {
$(this).parents('.base-win').addClass('hide');
});


$('.base-win').on('click', '.del', function() {


var $id = $(this).attr('data-relation-id');

$.ajax({
url : Url.ajaxDeleteRelationUrl,
data: {relation_id: $id, '_csrf-frontend': $('meta[name="csrf-token"]').attr('content')},
dataType: 'JSON',
type: 'post',
success: function (response) {
if(response.error === 0){
$(".base-win").addClass('hide');
$('.hover-album[data-ralation='+ $id +']').remove();
}else{
$(".base-win .pcon").html(response.message);
}
},
error: function (response) {
$(".base-win .pcon").html('操作失败，请刷新页面后重试');
},
});
});
...
* 上传
*/
function createUploadFunc(imagePolicy, imageSignature) {
var $ = jQuery,
state = 'pending',
$uploaderInput = $('#coverInput'),
$pickBtn = '#uploadCoverButton',
$img = $(".img-cover img"),
$uploadProgressContar = $(".progress");
...
$uploadProgressContar.removeClass('hide').css('background-color', 'rgb(195, 194, 194)').css('width', '(' + percentage * 217 + ')px');
$uploadProgressContar.html((percentage * 100).toFixed(2) + '%');
});

// 上传成功
uploader.on('uploadSuccess', function (file, response) {
$uploaderInput.val(response.url);
$uploadProgressContar.css('background-color', '#fff');
$img.attr('src', "http://pic.huke88.com/" + response.url);
});
...
// $uploadProgressContar.html('').addClass('hide');
});
...
$("form").find("input[name=title]").on('focus', function () {
$(this).css('border', '');
});


$(".return-btn").on('click', function () {
window.location.href= Url.backto;
});
...
if(title == ''){
$form.find("input[name=title]").css('border', '1px solid red');
return false;
}

var tags = [];
$(".tags .tag-r span").each(function () {
...
data = $.parseJSON(data);
if(data.error == '0'){
window.location.href = Url.successTo;
}else{
